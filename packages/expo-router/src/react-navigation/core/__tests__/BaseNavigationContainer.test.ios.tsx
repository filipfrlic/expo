...
test('emits option events when options change with tab router', () => {
  ...
  const element = (
    <BaseNavigationContainer
      ...
      <TestNavigator>
        <Screen name="foo" options={{ shared: true }}>
          {() => null}
        </Screen>
        <Screen name="bar" options={{ shared: true }}>
          {() => null}
        </Screen>
  ...
  expect(listener).toHaveBeenCalledTimes(1);
  expect(listener.mock.calls[0]![0].data.options).toEqual({ shared: true });
  expect(ref.current?.getCurrentOptions()).toEqual({ shared: true });
  ...
  expect(listener).toHaveBeenCalledTimes(2);
  expect(listener.mock.calls[1]![0].data.options).toEqual({ shared: true });
